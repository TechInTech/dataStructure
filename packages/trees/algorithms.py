...
from stackset.linkedstack import LinkedStack
from queueset.linkedqueue import LinkedQueue
from collection.grid import Grid
...
def breadthFisrtSearch(g, startLabel):
    result = list()
    queue = LinkedQueue()
    g.clearVertexMarks()
    queue.add(g.getVertex(startLabel))
    while not queue.isEmpty():
        ...
        vertex.setMark()
        result.append(vertex.getLabel())
        if v in g.neighboringVertices(vertex.getLabel()):
            if not v.isMarked():
                queue.add(v)
    return result
