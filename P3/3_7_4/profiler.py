...
p = Profiler()
p.test(selectionSort, size = 15, comp = True, exch = True, trace = False)
'''

import time
import random
from algorithms import selectionSort, bubbleSort
...
class Profiler(object):
    ...
    def test(self, function, lyst = None, size = 10,
             ...
             trace = True):
        '''
        function:  the algorithms being profiled
    ...
    def _stopClock(self):
        '''Stop the clock and compute the elapsed time
...
p.test(selectionSort, size = 7)
p.test(bubbleSort, size= 7)
